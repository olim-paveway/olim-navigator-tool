...
function buildFallbackPlan(data: FormSchema): AiPlanSchema {
  ...
  return {
    ...
    intent_band: band,
    personal_snapshot: `You are a ${data.familyType.toLowerCase()} from ${data.country} with your sights set on ${data.targetArea} — and your aliyah journey starts here. Based on your profile, you are in the "${band}" phase of your aliyah planning.`,
    profile_meaning: `As a ${data.career.toLowerCase()} ${data.familyType.toLowerCase()} from ${data.country}, your move to ${data.targetArea} is very achievable — ${data.country} is one of the better-supported origins for aliyah. Your career situation as a ${data.career.toLowerCase()} is a key factor in your financial planning and visa timing.`,
